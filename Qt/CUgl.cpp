...
//  Apply light
//
QVector3D CUgl::doLight(LightSource* lightSource)
{
   //  Light position
   float x = Lr*Cos(zh);
   float y = ylight;
   ...
   for(int i = 0; i < 3; i++){
       lightSource->ambient[i] = La;
       lightSource->diffuse[i] = Ld;
       lightSource->specular[i] = Ls;
   }
   lightSource->ambient[3] = 1.0;
   lightSource->diffuse[3] = 1.0;
   lightSource->specular[3] = 1.0;
   lightSource->position[0] = x;
   lightSource->position[1] = y;
   lightSource->position[2] = z;
   lightSource->position[3] = 1.0;
